...
export default class FinanceClient extends RestClient {
  constructor() {
    super('HMPPS Finance API', hmppsConfig.apis.finance, console, getHmppsAuthClient())
  }
   ...
   */
  async addHold(prisonId: string, offenderNo: string, holdRequest: AddHoldRequest): Promise<AddHoldResponse> {
  ...
  async releaseHold(
    ...
    return this.post<ReleaseHoldResponse>(
      {
        path: `/api/finance-holds/prison/${prisonId}/offenders/${offenderNo}/release-hold/${holdNumber}`,
        data: releaseRequest,
      },
      asSystem(),
    )
